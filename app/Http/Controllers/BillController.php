...
class BillController extends Controller
{
    ...
    public function create($vendorId)
    {
        ...
        if (\Auth::user()->can('create bill')) {
            ...
            return view('bill.create', compact('venders', 'bill_number', 'product_services', 'warehouses', 'category', 'customFields', 'vendorId'));
        } else {
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create bill')) {
            ...
            for ($i = 0; $i < count($products); $i++) {
                ...
                $billProduct->warehouse_id = $products[$i]['warehouse'];
                ...
                StockTransaction::create([
                    'dated' => $bill->bill_date,
                    'warehouse_id' => $billProduct->warehouse_id,
                    'item_id' => $billProduct->product_id,
                    'reference_id' => $bill->id,
                    'reference_line_id' => $billProduct->id,
                    'referee_id' => $bill->vender_id,
                    'reference_type' => Commons::BILL,
                    'quantity' => $billProduct->quantity,
                    'line_total' => $billProduct->quantity * $billProduct->price,
                    'created_at' => now()->utc(),
                ]);
            }
            DB::commit();
    ...
    public function edit($ids)
    {
        if (\Auth::user()->can('edit bill')) {
           ...
           $bill_number      = \Auth::user()->billNumberFormat($bill->bill_id);
            ...
            return view('bill.edit', compact('venders', 'product_services', 'warehouses', 'bill', 'bill_number', 'category', 'customFields'));
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }
    ...
    public function update(Request $request, Bill $bill)
    {
        if (\Auth::user()->can('edit bill')) {
            ...
            if ($bill->created_by == \Auth::user()->creatorId()) {
                ...
                StockTransaction::where('reference_id', $bill->id)
                    ->where('reference_type', Commons::BILL)
                    ->delete();
                ...
                for ($i = 0; $i < count($products); $i++) {
                    ...
                    $billProduct->warehouse_id = $products[$i]['warehouse_id'];
                    $billProduct->quantity = $products[$i]['quantity'];
                    $billProduct->tax = $products[$i]['tax'];
                    $billProduct->discount = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
                    $billProduct->price = $products[$i]['price'];
    ...
    public function productDestroy(Request $request)
    {
        if (\Auth::user()->can('delete bill product')) {
            BillProduct::where('product_id', '=', $request->id)->delete();
    ...
    public function venderBillShow($ids)
    {
        if (\Auth::user()->can('show bill')) {
            $bill_id = \Crypt::decrypt($ids);
            $bill = Bill::where('id', $bill_id)->first();
