...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (\Auth::user()->can('manage invoice')) {
            ...
            return view('invoice.index', compact('invoices', 'customer', 'user', 'status'));
        } else {
    ...
    public function create($customerId)
    {
        ...
        if (\Auth::user()->can('create invoice')) {
            ...
            return view('invoice.create', compact('customers', 'invoice_number', 'warehouses', 'product_services', 'category', 'customFields', 'customerId'));
        } else {
    ...
    public function product(Request $request)
    {
        ...
        {
            $data['onhand'][$warehouse->id] = StockTransaction::where('item_id', $product->id)
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create invoice')) {
            ...
            for ($i = 0; $i < count($products); $i++) {
                ...
                $invoiceProduct->warehouse_id = $products[$i]['warehouse_id'];
                ...
                StockTransaction::create([
                    'dated' => $invoice->issue_date,
                    'warehouse_id' => $invoiceProduct->warehouse_id,
                    'item_id' => $invoiceProduct->product_id,
                    'reference_id' => $invoice->id,
                    'reference_line_id' => $invoiceProduct->id,
                    'referee_id' => $invoice->customer_id,
                    'reference_type' => Commons::INVOICE,
                    'quantity' => -$invoiceProduct->quantity,
                    'line_total' => $invoiceProduct->quantity * $invoiceProduct->price,
                    'created_at' => now()->utc(),
                ]);
            }
    ...
    public function edit($ids)
    {
        if (\Auth::user()->can('edit invoice')) {
            ...
            return view('invoice.edit', compact('customers', 'product_services', 'warehouses', 'invoice', 'invoice_number', 'category', 'customFields'));
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }
    ...
    public function update(Request $request, Invoice $invoice)
    {
        if (\Auth::user()->can('edit invoice')) {
            if ($invoice->created_by == \Auth::user()->creatorId()) {
                ...
                StockTransaction::where('reference_id', $invoice->id)
                    ->where('reference_type', Commons::INVOICE)
                    ->delete();
 ...
 $user = User::get()->pluck('name', 'id');
            ...
            return view('invoice.index', compact('invoices', 'status','user'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
