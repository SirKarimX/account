...
class ReportController extends Controller
{
    public function stock(Request $request)
    {
        ...
        $warehouses->prepend('All', '');

        $product_services = ProductService::where('created_by', \Auth::user()->creatorId())
            ->orderBy('name')
            ->get()
            ->pluck('name', 'id');
        $product_services->prepend('All', '');

        $data['monthList'] = $month = $this->yearMonth();
        $data['yearList'] = $this->yearList();
        $filter['startDateRange'] = date('M-Y');
        $filter['endDateRange'] = date('M-Y');
        $filter['type'] = __('Stock Report');
        $filter['warehouse'] = __('All');
        $filter['product'] = __('All');

        $reportData = StockTransaction::with([
            'item',
            'warehouse',
            'invoice',
            'bill',
        ...
        if($request->has('start_month') && $request->has('end_month')) {
            ...
            $start_month = $request->get('start_month') . '-01';
            $end_month = $request->get('end_month') . '-' . Carbon::parse($request->get('end_month') . '-01')->daysInMonth;

            $from_date = Carbon::parse($start_month);
            $to_date = Carbon::parse($end_month);

             $filter['startDateRange'] = $from_date->format('M-Y');
             $filter['endDateRange'] = $to_date->format('M-Y');

            $reportData = $reportData->where(function($q) use($start_month, $end_month) {
                $q->whereDate('dated', '>=', $start_month)
                    ->whereDate('dated', '<=', $end_month);
            });
        }

        if($request->has('warehouse') && !empty($request->get('warehouse'))) {
            $reportData = $reportData->where('warehouse_id', $request->get('warehouse'));
            $filter['warehouse'] = Warehouse::find($request->get('warehouse'))->name;
        }

        if($request->has('product') && !empty($request->get('product'))) {
            $reportData = $reportData->where('item_id', $request->get('product'));
            $filter['product'] = ProductService::find($request->get('product'))->name;
        }
        ...
        return view('report.stock', compact('reportData','warehouses', 'product_services', 'filter', 'data'));
