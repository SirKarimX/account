...
class User extends Authenticatable
{
    ...
    public function adjustmentlNumberFormat($number)
    {
        $settings = Utility::settings();

        return $settings["adjustment_prefix"] . sprintf("%05d", $number);
    }
 ...
 public function userPayment($userId)
    {
        $payments  = Transaction:: where('user_id', $userId)->orderBy('date', 'desc')->get();
        

        return $payments;
    }
    public function userInvoice($userId)
    {
            $invoices  = Invoice:: where('user_id', $userId)->orderBy('issue_date', 'desc')->get();
        

        return $invoices;
    }
    
     public function userTotalInvoiceSum($userId)
    {
    ...
    public function userTotalPaymentInvoice($userId)
    {
        $month=now()->format('m');
        $year=now()->format('Y');
        $balance = InvoicePayment::where('user_id', '=', $userId)->whereYear('date', $year)
              ->whereMonth('date', $month)->get()->sum('amount');
     

        return $balance;
    }
   public function userTotalPaymentSum($userId)
    {
        $month=now()->format('m');
        $year=now()->format('Y');
        $balance = Payment::where('user_id', '=', $userId)->whereYear('date', $year)
              ->whereMonth('date', $month)->get()->sum('amount');
