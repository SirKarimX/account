    ...
    <div class="scrollbar-inner">
        <div class="div-mega">
            <ul class="navbar-nav navbar-nav-docs">
                ...
                @else
                    @if( Gate::check('manage user') || Gate::check('manage role'))
                        <li class="nav-item">
                            ...
                            <div
                                class="collapse {{ (Request::segment(1) == 'users' || Request::segment(1) == 'roles' || Request::segment(1) == 'permissions')?'show':''}}"
                                id="navbar-staff">
                ...
                @endif
                    ...
                    <li class="nav-item">
                        <a href="{{ route('employee.index') }}"
                           class="nav-link {{ (Request::segment(1) == 'employee')?'active':''}}">
                            <i class="fas fa-shopping-cart"></i>{{__('Employee')}}
                        </a>
                    </li>
                @endif
                ...
                <li class="nav-item">
                    <a class="nav-link {{ (Request::segment(1) == 'warehouse'
                       ...
                       aria-expanded="{{ (Request::segment(1) == 'warehouse' || Request::segment(1) == 'warehouse-transfer')?'true':'false'}}"
                       aria-controls="navbar-banking">
                        <i class="fas fa-warehouse"></i>{{__('Warehouses')}}
                        <i class="fas fa-sort-up"></i>
                    </a>
                    <div
                        class="collapse {{ (Request::segment(1) == 'warehouse' || Request::segment(1) == 'stock-transfer'
                                    || Request::segment(1) == 'stock-adjustment')?'show':''}}"
                        id="navbar-warehouses">
                        <ul class="nav flex-column submenu-ul">
                            @can('manage warehouse')
                            <li class="nav-item {{ (Request::route()->getName() == 'warehouse.index' || Request::route()->getName() == 'warehouse.create'
                                            || Request::route()->getName() == 'warehouse.edit') ? ' active' : '' }}">
                                <a href="{{route('warehouse.index')}}" class="nav-link">{{ __('Warehouses') }}</a>
                            </li>
                            @endcan
                             @can('stock warehouse')
                            <li class="nav-item {{ (Request::route()->getName() == 'warehouse.stock-report') ? ' active' : '' }}">
                                <a href="{{ route('warehouse.stock-report') }}"
                                   class="nav-link">{{ __('Warehouse Stock') }}</a>
                            </li>
                             @endcan
                             @can('transfer warehouse')
                            <li class="nav-item {{ (Request::route()->getName() == 'stock-transfer.index'
                                ...
                                <a href="{{ route('stock-transfer.index') }}"
                                   class="nav-link">{{ __('Stock Transfer') }}</a>
                            </li>
                            @endcan
                            @can('adj warehouse')
                            <li class="nav-item {{ (Request::route()->getName() == 'stock-adjustment.index'
                                ...
                                <a href="{{ route('stock-adjustment.index') }}"
                                   class="nav-link">{{ __('Stock Adjustment') }}</a>
                            </li>
                            @endcan
                        </ul>
                    </div>
                ...
                {{--<li class="nav-item">
                    <a href="{{ route('warehouse.index') }}" class="nav-link {{ (Request::segment(1) == 'warehouse')?'active':''}}">
                        <i class="fas fa-building"></i>{{__('Warehouses')}}
                    </a>
                </li>--}}
                @endif
                ...
                @if( Gate::check('manage bank account') ||  Gate::check('manage transfer'))
                    <li class="nav-item">
                        ...
                        <div
                            class="collapse {{ (Request::segment(1) == 'bank-account' || Request::segment(1) == 'transfer')?'show':''}}"
                            id="navbar-banking">
                            <ul class="nav flex-column submenu-ul">
                ...
                @if( Gate::check('manage invoice') ||  Gate::check('manage revenue') ||  Gate::check('manage credit note'))
                    <li class="nav-item">
                        ...
                        <div
                            class="collapse {{ (Request::segment(1) == 'invoice' || Request::segment(1) == 'revenue' || Request::segment(1) == 'credit-note')?'show':''}}"
                            id="navbar-income">
                            <ul class="nav flex-column submenu-ul">
                                @can('manage invoice')
                ...
                @if( Gate::check('manage bill')  ||  Gate::check('manage payment') ||  Gate::check('manage debit note'))
                    <li class="nav-item">
                        ...
                        <div
                            class="collapse {{ (Request::segment(1) == 'bill' || Request::segment(1) == 'payment' || Request::segment(1) == 'debit-note'  )?'show':''}}"
                            id="navbar-expense">
                            <ul class="nav flex-column submenu-ul">
                                @can('manage bill')
                ...
                @if( Gate::check('manage chart of account') ||  Gate::check('manage journal entry') ||   Gate::check('balance sheet report') ||  Gate::check('ledger report') ||  Gate::check('trial balance report'))
                    <li class="nav-item">
                        ...
                        <div
                            class="collapse {{ (Request::segment(1) == 'chart-of-account'  || Request::segment(1) == 'journal-entry' || Request::segment(2) == 'ledger' ||  Request::segment(2) == 'balance-sheet' ||  Request::segment(2) == 'trial-balance')?'show':''}}"
                ...
                @if( Gate::check('income report') || Gate::check('expense report') || Gate::check('income vs expense report') || Gate::check('tax report')  || Gate::check('loss & profit report') || Gate::check('invoice report') || Gate::check('bill report') || Gate::check('invoice report') ||  Gate::check('manage transaction')||  Gate::check('statement report'))
                    <li class="nav-item">
                        <a class="nav-link {{ ((Request::segment(1) == 'report' || Request::segment(1) == 'transaction')
                        ...
                        <div
                            class="collapse {{ ((Request::segment(1) == 'report' || Request::segment(1) == 'transaction') &&  Request::segment(2) == 'stock' &&  Request::segment(2) != 'ledger' &&  Request::segment(2) != 'balance-sheet' &&  Request::segment(2) != 'trial-balance')?'show':''}}"
                            id="navbar-reports">
                            <ul class="nav flex-column submenu-ul">
                                ...
                                @endcan
                                <li class="nav-item {{ (Request::route()->getName() == 'report.stock') ? ' active' : '' }}">
                                    <a href="{{route('report.stock')}}"
                                       class="nav-link">{{ __('Stock Report') }}</a>
                                </li>
                ...
                @if(Gate::check('manage constant tax') || Gate::check('manage constant category') ||Gate::check('manage constant unit') ||Gate::check('manage constant payment method') ||Gate::check('manage constant custom field') )
                    <li class="nav-item">
                        <a class="nav-link {{ (Request::segment(1) == 'taxes' || Request::segment(1) == 'product-category' || Request::segment(1) == 'product-unit' || Request::segment(1) == 'payment-method' || Request::segment(1) == 'custom-field' || Request::segment(1) == 'chart-of-account-type')?' active':'collapsed'}}"
                           href="#navbar-constant" data-toggle="collapse" role="button" aria-expanded="{{ (Request::segment(1) == 'taxes' || Request::segment(1) == 'product-category' || Request::segment(1) == 'product-unit' || Request::segment(1) ==
                        ...
                        <div
                            class="collapse {{ (Request::segment(1) == 'taxes' || Request::segment(1) == 'product-category' || Request::segment(1) == 'product-unit' || Request::segment(1) == 'payment-method' || Request::segment(1) == 'custom-field' || Request::segment(1) == 'chart-of-account-type')?'show':''}}"
                            id="navbar-constant">
                            <ul class="nav flex-column submenu-ul">
