...
@push('script-page')
    ...
    <script>
        ...
        $(document).on('change', '.item', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $(el).parent().attr('data-quantities', JSON.stringify(item.onhand));
        ...
        function check_for_negative_qty()
        {
            let invalid_qty = false;

            $('.invoice_row').each(function(n, v) {
                let quantities = JSON.parse($(v).find('.item-cell').attr('data-quantities'));
                let qty_box= $(v).find('.quantity');
                let warehouse_id = $(v).find('.warehouse').val();
                //user provided a correct warehouse and numeric value in qty field
                if(warehouse_id.trim().length > 0 && qty_box.val().trim().length > 0 && !isNaN(qty_box.val())) {

                    for(let key in quantities) {

                        qty_box.attr('title', `Only ${quantities[key]} quantity left in this warehouse.`);

                        if(key == warehouse_id && parseFloat(qty_box.val()) > parseFloat(quantities[key])) {
                            qty_box.addClass('is-invalid');

                            invalid_qty = true;
                            break;
                        }
                        else {
                            qty_box.removeClass('is-invalid');
                        }
                    }
                }
            });
            ...
            if(invalid_qty) {
                alert("Not enough stock is available for the highlighted products");
                return false;
            }
...
@section('content')
    <div class="row">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped mb-0" data-repeater-list="items" id="sortable-table">
                            ...
                            <tr class="invoice_row">
                                <td width="25%" class="item-cell">
                                    {{ Form::select('item', $product_services,'', array('class' => 'form-control select2 item','data-url'=>route('invoice.product'),'required'=>'required')) }}
                                </td>
                                <td>
                                    {{ Form::select('warehouse_id', $warehouses,'', array('class' => 'warehouse form-control select2','required'=>'required')) }}
                                </td>
                                <td>
        ...
        <div class="col-12 text-right">
            <input type="submit" id="btn-submit" value="{{__('Create')}}" onclick="return check_for_negative_qty();" class="btn-create btn-xs badge-blue radius-10px">
